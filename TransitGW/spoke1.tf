...
resource "local_file" "ssh_key_ue1" {
    # do not include aws provider or aws alias provider
  filename = "${aws_key_pair.ec2_keypair_ssh_ue1.key_name}.pem"
...
resource "aws_route" "tgwAttach-route-1-toSpoke2pub" {
  provider = aws.virginia
  route_table_id            = data.aws_route_table.rt-pub.id
  destination_cidr_block    = "10.2.0.0/16"
  transit_gateway_id = aws_ec2_transit_gateway.tgw.id 
}
resource "aws_route" "tgwAttach-route-1-toSpoke3pub" {
  provider = aws.virginia
  route_table_id            = data.aws_route_table.rt-pub.id
